...
class Config:
    def __init__(self, configfile=SETTINGS):
        ...
        files = [
            "/etc/sentinelsd/%s" % configfile,
            "%s/.%s" % (expanduser("~"), configfile),
            configfile,
        ]
        print(
            "** We hope you enjoy using Sentinels. For more open source Canary goodness, head over to canarytokens.org. **"
        )
        for fname in files:
            try:
        ...
        if self.__config is None:
            print(
                'No config file found. Please create one with "sentinelsd --copyconfig"'
            )
            sys.exit(1)
