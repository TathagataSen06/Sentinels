#!/bin/bash -e
# Build sentinels package from git repo and create virtualenv where it can be run.
# Tested on macOS but should work elsewhere as long as the compile flags are configured.
#
# Environment variable options:
#    SENTINELS_BUILD_FULL_CLEAN=True - do a new git checkout
#    SENTINELS_BUILD_FRESH_VENV=True - recreate the venv

# Configurable variables
SENTINELS_GIT_REPO="https://github.com/thinkst/sentinels.git"
...
SENTINELS_DIR="$(readlink -f -- "$BUILD_SCRIPT_DIR/../";)"
BUILD_LOG="$BUILD_SCRIPT_DIR/build.log"
VENV_PATH="$SENTINELS_DIR/$VENV_DIR"
USE_UV=0
...
if [[ "${SENTINELS_BUILD_FULL_CLEAN+set}" == set ]]; then
    echo_log "SENTINELS_BUILD_FULL_CLEAN requested; backing up repo and rebuilding from scratch..."
    pushd .. >> "$BUILD_LOG"
    [[ -a sentinels ]] && mv_to_old sentinels
    git clone "$SENTINELS_GIT_REPO"
    popd >> "$BUILD_LOG"
else
    echo_log "Using current repo at '$SENTINELS_DIR'"
    echo "    (Set SENTINELS_BUILD_FULL_CLEAN=true to start from a fresh git checkout)"
fi


# Backup current virtual env and make a new one if requested
if [[ -d $VENV_PATH ]]; then
    if [[ "${SENTINELS_BUILD_FRESH_VENV+set}" == set ]]; then
        mv_to_old "$VENV_PATH"
        create_venv
    else
        echo_log "Using current virtualenv in '$VENV_PATH'"
        echo "    (Set SENTINELS_BUILD_FRESH_VENV=true to rebuild a new virtualenv)"
    fi
else
    create_venv
fi
...
BUILT_PKG=`ls -t dist/sentinels*.tar.gz | head -n 1`
BUILT_PKG_COUNT=`ls -1 dist/sentinels*.tar.gz | wc -l | tr -d ' '`
...
echo_log "\n    . '$SENTINELS_DIR/env/bin/activate'\n\n"
popd >> "$BUILD_LOG"
