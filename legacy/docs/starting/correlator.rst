...
Inside the virtualenv, install Sentinels Correlator following the instructions in the `README <https://github.com/thinkst/sentinels-correlator>`_.

The correlator runs with a default config, which we'll copy and edit to get started.

.. code-block:: sh

   $ sentinels-correlator
   Warning: no config file specified. Using the template config:
   /[...]/sentinels_correlator.conf
   $ cp /[...]/sentinels_correlator.conf sentinels-correlator.conf
...
.. code-block:: json

   {
     ...
     "console.email_notification_address": ["notifications@sentinels.org"],
...
.. code-block:: sh
   ...
   $ sentinels-correlator --config=./sentinels-correlator.conf

To configure Sentinels daemons to send their events to the correlator, edit the **logger** field in its config and restart the daemon to reload the config.
...
You can test that the Correlator alerts are working by sending an event directly to it (without using Sentinels).
