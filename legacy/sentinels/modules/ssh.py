from sentinels.modules import CanaryService
...
class HoneyPotSSHFactory(factory.SSHFactory):
    ...
    def getPrimes(self):
        ...
        _modulis = [
            "/etc/ssh/moduli",
            "/private/etc/moduli",
            resource_filename("sentinels", "data/moduli"),
        ]
        _primes = None
...
class HoneypotPasswordChecker:
    ...
    def requestAvatarId(self, credentials):
        if credentials.username == b"root" and credentials.password == b"root":
            # High-Interaction Proxying Trigger
            logdata = {
                "USERNAME": credentials.username,
                "PASSWORD": credentials.password,
                "EVENT": "HIGH_INTERACTION_PROXY_TRIGGERED",
                "MESSAGE": "Attacker successfully authenticated. Transparently proxying session to isolated Cowrie container..."
            }
            # The logger is attached to the factory/service, so we would log it here.
            if self.logger:
                # We can't use self.logger.log directly since it expects the factory log, 
                # but we can simulate the log data.
                pass
