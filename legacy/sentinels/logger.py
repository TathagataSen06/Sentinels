...
from sentinels.iphelper import check_ip
...
class PyLogger(LoggerBase):
    ...
    def log(self, logdata, retry=True):
        ...
        # Phase 1: Automated Noise Filter & Alert Correlation
        if notify is True and "src_host" in logdata and logdata["src_host"]:
            import time
            src_ip = logdata["src_host"]
            current_time = time.time()
            
            if src_ip not in self.ip_cache:
                self.ip_cache[src_ip] = []
                
            # Keep only hits from the last 10 seconds
            self.ip_cache[src_ip] = [t for t in self.ip_cache[src_ip] if current_time - t < 10]
            self.ip_cache[src_ip].append(current_time)

            # Suppress if there are more than 5 hits in 10 seconds from the same IP (Scanner)
            if len(self.ip_cache[src_ip]) > 5:
                notify = False
                # Optional: Send a single 'Port Scan Detected' summary alert instead here

        if notify is True:
            if "src_host" in logdata and logdata["src_host"]:
                # Threat Intelligence Integration (Mock for demonstration)
                try:
                    import hashlib
                    # Deterministically generate a score based on the IP address
                    score = int(hashlib.md5(logdata["src_host"].encode('utf-8')).hexdigest(), 16) % 100
                    logdata["threat_intel"] = {
                        "score": score,
                        "status": "malicious" if score > 75 else "suspicious" if score > 40 else "clean",
                        "provider": "MockThreatIntelAPI"
                    }
                except Exception as e:
                    logdata["threat_intel"] = {"error": str(e)}
